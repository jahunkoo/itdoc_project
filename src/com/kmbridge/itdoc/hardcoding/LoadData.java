...
public class LoadData {
	...
	private void createKeywordMap() {
		...
		keywordMap.put(7, "비만한약,장쾌해독,변비치료,교통사고,공진단,만성피로");
		keywordMap.put(8, "한방 다이어트,지방분해 식욕억제 다이어트한약,한약재");
		keywordMap.put(9, "면역치료,류마티스관절염,베체트병,산후풍,구내염,강직성척추염,소화기질환,아이");
		keywordMap.put(10, "성기능장애,조루증,발기부전,성불감증,불임,내과,당뇨");
		...
		keywordMap.put(17, "위대장,호흡기,피부 질환,24체질연구소,체칠침,내과,위장");
		...
		keywordMap.put(21, "위염,역류성식도염,신경성소화불량,만성장염,위장,당뇨");
		keywordMap.put(22, "천식,폐질환,대장염,아토피,피부염,아이");
		keywordMap.put(23, "질염,방광염,질건조증,산후몸조리,한방성형,아이");
		...
		keywordMap.put(30, "비수술 척추,체형,골반교정,턱관절,일자목,산후관리,교통사고후유증,추나요법,아이,내과");
	}
	...
	public KmClinicDetailView getKmClinicDetailView(int kmClinicId) {
		...
		try {
			...
			/*
			 * switch(kmClinicId){ case 1:
			 * view.setPicturePath("biman_bosung.png");break; case 2:
			 * view.setPicturePath("biman_bosung.png");break; case 7:
			 * view.setPicturePath("biman_godangbi.png");break; //고당비한의원id: 7
			 * case 11: view.setPicturePath("biman_dongbang.png");break;
			 * //동방한의원id: 11 case 14:
			 * view.setPicturePath("body_miache.png");break; //미아체한의원 : 14 case
			 * 30: view.setPicturePath("body_kyunghee.png");break; //청구경희한의원: 30
			 * }
			 */
			...
			if (jsonObj.has("doctorList")) {
				...
				for (int j = 0; j < jsonArr.length(); j++) {
					...
					if (innerJson.has("id"))
						doctor.setId(innerJson.getInt("id"));
					if (innerJson.has("kmClinicId"))
						doctor.setKmClinicId(innerJson.getInt("kmClinicId"));
					if (innerJson.has("name"))
						doctor.setName(innerJson.getString("name"));
					if (innerJson.has("career"))
						doctor.setCareer(innerJson.getString("career"));
					if (innerJson.has("major"))
						doctor.setMajor(innerJson.getString("major"));
					if (innerJson.has("academy"))
						doctor.setAcademy(innerJson.getString("academy"));
